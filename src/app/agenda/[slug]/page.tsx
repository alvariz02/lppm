...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const item = await db.agenda.findUnique({
    where: { slug },
...
export default async function AgendaDetailPage({ params }: PageProps) {
  ...
  const eventTypeLabel =
    AGENDA_EVENT_TYPE_LABELS[agenda.eventType as keyof typeof AGENDA_EVENT_TYPE_LABELS] ||
    agenda.eventType ||
    'Kegiatan'
  const statusLabel =
  ...
  const relatedAgenda = await db.agenda.findMany({
    where: {
      id: { not: agenda.id },
              ...
              <Card className="border-0 shadow-md">
                ...
                <CardContent className="space-y-3">
                  <div>
                    ...
                    <p className="text-sm font-medium">{formatDateTime(agenda.startDate)}</p>
                  </div>
                  {agenda.endDate && (
                    <div>
                      <p className="text-xs text-muted-foreground">Tanggal Selesai</p>
                      <p className="text-sm font-medium">{formatDateTime(agenda.endDate)}</p>
                    </div>
                  )}
                  <Separator />
                  <div>
        ...
        <section className="py-10 bg-muted/30">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {relatedAgenda.map((item) => {
                const relStatusLabel =
                  AGENDA_STATUS_LABELS[item.status as keyof typeof AGENDA_STATUS_LABELS] ||
                  item.status
                const relStatusColor =
                  AGENDA_STATUS_COLORS[item.status as keyof typeof AGENDA_STATUS_COLORS] || ''
                const relEventType =
                  AGENDA_EVENT_TYPE_LABELS[item.eventType as keyof typeof AGENDA_EVENT_TYPE_LABELS] ||
                  item.eventType ||
                  'Kegiatan'
                return (

...
import { FolderOpen, Plus, Pencil, Trash2, Search, Loader2 } from 'lucide-react'
...
export default function AdminDocumentCategoriesPage() {
  ...
  const [editingItem, setEditingItem] = useState<DocumentCategoryItem | null>(null)
  const [deletingItem, setDeletingItem] = useState<DocumentCategoryItem | null>(null)
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['admin-document-categories', search, page, pageSize],
    queryFn: async () => {
      ...
      const res = await fetch(`/api/admin/document-categories?${params}`)
      if (!res.ok) throw new Error('Gagal memuat data')
      return res.json() as Promise<PaginatedResponse<DocumentCategoryItem>>
    },
  })
  ...
  const createMutation = useMutation({
    mutationFn: async (values: CategoryFormValues) => {
      ...
      if (!res.ok) {
        ...
        throw new Error(err.error || 'Gagal membuat kategori dokumen')
      }
      return res.json()
    },
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, values }: { id: string; values: CategoryFormValues }) => {
      ...
      if (!res.ok) {
        const err = await res.json()
        throw new Error(err.error || 'Gagal memperbarui kategori dokumen')
      }
      return res.json()
    },
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      ...
      if (!res.ok) {
        const err = await res.json()
        throw new Error(err.error || 'Gagal menghapus kategori dokumen')
      }
      return res.json()
    },
    onSuccess: () => {
      toast.success('Kategori dokumen berhasil dihapus')
      queryClient.invalidateQueries({ queryKey: ['admin-document-categories'] })
      setDeleteOpen(false)
  ...
  const openDeleteDialog = useCallback((cat: DocumentCategoryItem) => {
    setDeletingItem(cat)
    setDeleteOpen(true)
  }, [])
    ...
    <motion.div initial="hidden" animate="visible" variants={container} className="space-y-6">
      ...
      <motion.div variants={item} className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-bold text-foreground">Kategori Dokumen</h2>
            <p className="text-sm text-muted-foreground">Kelola kategori dokumen unduhan</p>
          </div>
        </div>
        {canWrite && (
          ...
          <CardContent className="p-0">
            {isLoading ? (
              ...
              <div className="text-center py-12 text-muted-foreground">
                ...
                <p className="font-medium">Belum ada data kategori dokumen</p>
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead className="text-center">Jumlah Dokumen</TableHead>
      ...
      <Dialog open={formOpen} onOpenChange={setFormOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{editingItem ? 'Edit Kategori Dokumen' : 'Tambah Kategori Dokumen'}</DialogTitle>
            <DialogDescription>
              {editingItem
                ? 'Perbarui nama kategori dokumen. Slug akan otomatis diperbarui.'
                : 'Masukkan nama kategori dokumen. Slug akan otomatis dibuat.'}
            </DialogDescription>
          </DialogHeader>
          <Form {...form}>
      ...
      <AlertDialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Hapus Kategori Dokumen</AlertDialogTitle>
            <AlertDialogDescription>

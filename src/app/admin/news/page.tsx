...
import { Plus, Pencil, Trash2, Search, Newspaper, ChevronLeft, ChevronRight, Star } from 'lucide-react'
...
import type { PaginatedResponse, News, NewsCategory } from '@/types'

// ============ TYPES ============

interface NewsRow extends News {
  category?: { id: string; name: string } | null
}

// ============ API HELPERS ============

async function fetchNews(page: number, pageSize: number, search: string) {
  const params = new URLSearchParams({ page: String(page), pageSize: String(pageSize) })
  if (search) params.set('search', search)
  const res = await fetch(`/api/admin/news?${params}`)
  if (!res.ok) throw new Error('Gagal memuat data berita')
  return res.json() as Promise<PaginatedResponse<NewsRow>>
}

async function fetchCategories() {
  const res = await fetch('/api/news?pageSize=100')
  if (!res.ok) return []
  // The public news API might not return categories, so we fetch from a dedicated endpoint
...
async function fetchNewsCategories() {
  try {
    ...
    return (json.data ?? []) as NewsCategory[]
  } catch {
    return [] as NewsCategory[]
  }
}

async function saveNews(data: NewsInput & { id?: string }) {
  const isEdit = !!data.id
  const url = isEdit ? `/api/admin/news/${data.id}` : '/api/admin/news'
  const method = isEdit ? 'PUT' : 'POST'
  const { id: _id, ...payload } = data as NewsInput & { id?: string }
...
export default function NewsAdminPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['admin-news', page, pageSize, search],
    queryFn: () => fetchNews(page, pageSize, search),
  })
  ...
  const saveMutation = useMutation({
    mutationFn: saveNews,
    onSuccess: () => {
      toast.success(editingItem ? 'Berita berhasil diperbarui' : 'Berita berhasil ditambahkan')
  ...
  function openEditDialog(item: NewsRow) {
    setEditingItem(item)
    form.reset({
      title: item.title,
      ...
      status: item.status as NewsInput['status'],
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-bold text-foreground">Berita</h2>
            <p className="text-sm text-muted-foreground">Kelola berita dan artikel</p>
          </div>
        </div>
        {canWrite && (
          <Button onClick={openCreateDialog} className="shrink-0">
            <Plus className="size-4 mr-1" /> Tambah Berita
          </Button>
        )}
      </div>
                      ...
                      <TableRow key={item.id}>
                        ...
                        <TableCell>
                          <Badge className={`text-[10px] ${getStatusColor(item.status)}`}>
                            {NEWS_STATUS_LABELS[item.status as keyof typeof NEWS_STATUS_LABELS] || item.status}
                          </Badge>
                        </TableCell>
                        <TableCell>
                          {item.isFeatured ? (
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="sm:col-span-2 space-y-1.5">
                <Label>Judul *</Label>
                <Input {...form.register('title')} placeholder="Judul berita" />
              ...
              <div className="space-y-1.5">
                <Label>Status *</Label>
                <Select value={form.watch('status')} onValueChange={(val) => form.setValue('status', val as NewsInput['status'])}>
              ...
              <div className="sm:col-span-2 space-y-1.5">
                ...
                <Textarea {...form.register('excerpt')} placeholder="Ringkasan berita" rows={2} />
              </div>

              <div className="sm:col-span-2 space-y-1.5">
                <Label>Konten *</Label>
                <Textarea {...form.register('content')} placeholder="Isi berita" rows={6} />
              ...
              <div className="space-y-1.5">
                ...
                <Input {...form.register('seoDescription')} placeholder="SEO description" />
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                />
                <Label htmlFor="isFeatured">Berita Featured</Label>
              </div>
            </div>

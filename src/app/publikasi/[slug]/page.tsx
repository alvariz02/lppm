...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const item = await db.publication.findUnique({
    where: { slug },
...
export default async function PublikasiDetailPage({ params }: PageProps) {
  ...
  const publication = await db.publication.findUnique({
    where: { slug },
    include: {
      research: { select: { id: true, title: true, slug: true, year: true } },
      service: { select: { id: true, title: true, slug: true, year: true } },
      publicationAuthors: {
        include: {
          researcher: {
            select: {
  ...
  if (!publication || !publication.isPublished) return notFound()
      ...
      <nav className="bg-muted/50 border-b">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-3">
          <ol className="flex items-center gap-2 text-sm text-muted-foreground flex-wrap">
            ...
            <li>/</li>
            <li>
              <Link href="/publikasi" className="hover:text-primary transition-colors">
                Publikasi
              </Link>
            </li>
            <li>/</li>
              ...
              {publication.publicationAuthors.length > 0 ? (
                <Card className="border-0 shadow-md">
                  ...
                  <CardContent className="space-y-2">
                    {publication.publicationAuthors.map((pa, index) => (
                      <div key={pa.id} className="flex items-center gap-3 p-2.5 rounded-lg bg-muted/50">
                        <div className="size-10 rounded-full bg-primary flex items-center justify-center text-primary-foreground font-bold text-xs shrink-0">
                          {getInitials(pa.researcher.name)}
                        </div>
                        <div className="min-w-0">
                          <p className="text-sm font-medium text-foreground truncate">
                            {index + 1}. {pa.researcher.name}
                            {pa.researcher.degree && (
                          ...
                          {pa.researcher.nidn && (
                            <p className="text-xs text-muted-foreground">NIDN: {pa.researcher.nidn}</p>
                          )}
                          {pa.researcher.expertise && (
                            <div className="flex flex-wrap gap-1 mt-1">
                              {pa.researcher.expertise.split(',').map((exp, i) => (
                ...
                <Card className="border-0 shadow-md">
                  ...
                  <CardContent>
                    <p className="text-muted-foreground leading-relaxed">{publication.authors}</p>
                  </CardContent>
                </Card>
              ) : null}
              ...
              <Card className="border-0 shadow-md">
                ...
                <CardContent>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {publication.journalName && (
                      <div>
                        <p className="text-xs text-muted-foreground">Nama Jurnal</p>
                        <p className="text-sm font-medium">{publication.journalName}</p>
                      </div>
                    )}
                    {publication.publisherName && (
                      <div>
                        <p className="text-xs text-muted-foreground">Penerbit</p>
                        <p className="text-sm font-medium">{publication.publisherName}</p>
                      </div>
                    )}
                    {publication.volume && (
                      <div>
                    ...
                    {publication.number && (
                      <div>
                        ...
                        <p className="text-sm font-medium">{publication.number}</p>
                      </div>
                    )}
                    {publication.pages && (
                      <div>
                        <p className="text-xs text-muted-foreground">Halaman</p>
                        <p className="text-sm font-medium">{publication.pages}</p>
                      </div>
                    )}
                    {publication.issn && (
                      <div>
                        <p className="text-xs text-muted-foreground">ISSN</p>
                        <p className="text-sm font-medium">{publication.issn}</p>
                      </div>
                    )}
                    {publication.isbn && (
                      <div>
                        <p className="text-xs text-muted-foreground">ISBN</p>
                        <p className="text-sm font-medium">{publication.isbn}</p>
                      </div>
                    )}
                    {publication.indexing && (
                      <div>
                        <p className="text-xs text-muted-foreground">Indeksasi</p>
                        <p className="text-sm font-medium">{publication.indexing}</p>
                      </div>
                    )}
                    {publication.accreditation && (
                      <div>
                        <p className="text-xs text-muted-foreground">Akreditasi</p>
                        <Badge variant="secondary" className="text-xs">{publication.accreditation}</Badge>
                      </div>
                    )}
                  </div>
                    ...
                    <>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="text-sm text-primary hover:underline flex items-center gap-1 line-clamp-1"
                        >
                          {publication.url}
                ...
                <Card className="border-0 shadow-md">
                  <CardHeader>
                    <CardTitle className="flex items-center gap-2 text-lg">
                      ...
                      Penelitian/Pengabdian Terkait
                    </CardTitle>
                  </CardHeader>
                  <CardContent className="space-y-3">
                    {publication.research && (
                      <Link
                        href={`/penelitian/${publication.research.slug}`}
                        className="block p-3 rounded-lg border hover:border-primary/30 hover:bg-primary/5 transition-colors"
                      >
                        <div className="flex items-center gap-2 mb-1">
                          <FlaskConical className="size-4 text-primary" />
                          <span className="text-xs text-muted-foreground">Penelitian</span>
                          <Badge variant="outline" className="text-[10px]">
                            {publication.research.year}
                          </Badge>
                        </div>
                    ...
                    {publication.service && (
                      <Link
                        href={`/pengabdian/${publication.service.slug}`}
                        className="block p-3 rounded-lg border hover:border-primary/30 hover:bg-primary/5 transition-colors"
                      >
                        <div className="flex items-center gap-2 mb-1">
                          <HeartHandshake className="size-4 text-sky-500" />
                          <span className="text-xs text-muted-foreground">Pengabdian</span>
                          <Badge variant="outline" className="text-[10px]">
                            {publication.service.year}
                          </Badge>
                        </div>
                        <p className="font-medium text-foreground text-sm line-clamp-2">
                          {publication.service.title}
                        </p>
                      </Link>
                    )}
                  </CardContent>
              ...
              <Card className="border-0 shadow-md">
                ...
                <CardContent className="space-y-3">
                  <div>
                    ...
                    <Badge variant="secondary">{pubTypeLabel}</Badge>
                  </div>
                  <div>
